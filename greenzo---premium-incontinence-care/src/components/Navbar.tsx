...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 w-full z-50 bg-brand-cream/90 backdrop-blur-md border-b border-black/5">
      <div className="max-w-7xl mx-auto px-10 md:px-16 h-24 flex items-center justify-between">
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2">
            <span className="text-2xl font-bold tracking-tighter uppercase text-brand-green">Greenzo</span>
            <span className="text-xl font-light opacity-60 ml-1 hidden sm:block">莞众</span>
          </div>
        ...
        <div className="hidden md:flex items-center gap-12">
          <ul className="flex gap-8 text-[11px] uppercase tracking-[0.25em] font-sans font-bold text-black/50">
            <li className="group">
          ...
          <div className="flex gap-4 text-[10px] font-sans font-bold text-black/30 border-l border-black/10 pl-12 h-6 items-center">
            {languages.map((lang) => (
              <button
                key={lang.code}
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="absolute top-20 left-0 w-full bg-white border-b border-zinc-100 p-6 flex flex-col gap-4 md:hidden"
          >
