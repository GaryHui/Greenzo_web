...
export default function QualitySection() {
  ...
  return (
    <section className="py-40 bg-brand-muted border-y border-black/5 overflow-hidden">
          ...
          <div className="lg:col-span-8 grid md:grid-cols-3 gap-8">
            {features.map((f, i) => (
              <motion.div
                ...
                className="bg-brand-cream border border-black/5 p-10 flex flex-col items-center text-center group hover:bg-brand-green transition-all duration-700 cursor-default"
              >
                <f.icon className="w-8 h-8 text-brand-green mb-8 group-hover:text-white transition-colors duration-700 stroke-[1.5px]" />
                <h3 className="text-xl font-serif text-brand-dark mb-4 group-hover:text-white transition-colors duration-700">{f.title}</h3>
